...
{
    public partial class FrmCell2 : Form
    {
        ...
        private void FrmCell2_Load(object sender, EventArgs e)
        {
            CGlobal.cellpo2 = "";
            CallPO();
            CallLoad();
        ...
        private void CallLoad()
        {
            ...
            try
            {
                ...
                Cmd = new System.Data.SqlClient.SqlCommand(" Select SUM(QtyBom) As QtyBom, SUM(QtyOut) As QtyOut ,SUM(QtyBal) As QtyBal  from dbo.DocMODtl  where   DocNo ='" + CGlobal.cellpo2 + "' and DeptCode='W8' and Barcode <> '' and  BomNo In ('CELL','STD')", conn);
                conn.Open();
                rs = Cmd.ExecuteReader();
                while (rs.Read())
                {
                    lblPO.Text = CGlobal.cellpo2;
