...
{
    public partial class FrmCell0 : Form
    {
        ...
        private void FrmCell0_Load(object sender, EventArgs e)
        {
            CallPO();
            CallLoad();
            CallTargettotal();
        ...
        private void CallLoad()
        {
            ...
            try
            {
                ...
                Cmd = new System.Data.SqlClient.SqlCommand(" Select SUM(QtyBom) As QtyBom, SUM(QtyOut) As QtyOut ,SUM(QtyBal) As QtyBal  from dbo.DocMODtl  where   DocNo ='" + CGlobal.cellpo00 + "' and DeptCode='W8' and Barcode <> '' and  BomNo In ('CELL','STD')", conn);
                conn.Open();
                rs = Cmd.ExecuteReader();
                while (rs.Read())
                {
