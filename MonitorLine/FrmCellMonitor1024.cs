...
{
    public partial class FrmCellMonitor1024 : Form
    {
        ...
        private void FrmCellMonitor1024_Load(object sender, EventArgs e)
        {
        ...
        private void Callday(string tempweek)
        {
            ...
            try
            {
                ...
                string strSQL1 = "";
                strSQL1 = " select * from Line_MonitorWeek  where Week='" + tempweek + "' order by ID";
                if (Isfind1 == true)
                {
        ...
        private void CallWeek(String week)
        {
            ...
            try
            {
                Cmd = new System.Data.SqlClient.SqlCommand(" SELECT sum(Totaltarget)as target,SUM(TotalOutput) as output from Line_MonitorWeek  where Week ='" + week + "' and week IS NOT NULL  group by week ", conn);
                conn.Open();
                rs = Cmd.ExecuteReader();
                while (rs.Read())
                {
                    CGlobal.WeekTarget = Convert.ToDouble(rs["target"].ToString()).ToString("#,####");
        ...
        private void CallMonthWeek(string month)
        {
            ...
            {
                string strSQL1 = "";
                strSQL1 = "  select week from Line_MonitorWeek  where SUBSTRING(tagetdate,4,2) ='" + month + "' and week IS NOT NULL  group by week ";
        ...
        private void CallYear(String month)
        {
            ...
            try
            {
                Cmd = new System.Data.SqlClient.SqlCommand(" select  SUM(Totaltarget) As target,SUM(TotalOutput) As Actual from dbo.Line_MonitorWeek where SUBSTRING(tagetdate,4,2) ='" + month  + "' ", conn);
                conn.Open();
                rs = Cmd.ExecuteReader();
                while (rs.Read())
                {
                    ...
                    {
                        this.MarT.Text = Convert.ToDouble(rs["target"].ToString()).ToString("#####");
                        this.MarA.Text = Convert.ToDouble(rs["Actual"].ToString()).ToString("#####");
                    }
                    else if (month == "04")
                    {
                        this.AprT.Text = Convert.ToDouble(rs["target"].ToString()).ToString("#####");
                        this.AprA.Text = Convert.ToDouble(rs["Actual"].ToString()).ToString("#####");
                    }
                    else if (month == "05")
                    {
                        this.MayT.Text = Convert.ToDouble(rs["target"].ToString()).ToString("#####");
                        this.MayA.Text = Convert.ToDouble(rs["Actual"].ToString()).ToString("#####");
                    }
                    else if (month == "06")
                    {
                        this.JunT.Text = Convert.ToDouble(rs["target"].ToString()).ToString("#####");
                        this.JunA.Text = Convert.ToDouble(rs["Actual"].ToString()).ToString("#####");
                    }
                    else if (month == "07")
                    {
